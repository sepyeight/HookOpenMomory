...
//xposed函数：
//std::unique_ptr<const DexFile> DexFile::OpenMemory(const uint8_t* base, size_t size, const std::string& location, uint32_t location_checksum, MemMap* mem_map, const OatDexFile* oat_dex_file, std::string* error_msg)
...
void hook_21_22() {
    ...
    if (addr == NULL) {
        __android_log_print(ANDROID_LOG_ERROR, TAG,
                            "Error: unable to find the Symbol : _ZN3art7DexFile10OpenMemoryEPKhjRKNSt3__112basic_stringIcNS3_11char_traitsIcEENS3_9allocatorIcEEEEjPNS_6MemMapES2_PS9_");
        return;
    }

    if (registerInlineHook((uint32_t) addr, (uint32_t) new_openmemory, (uint32_t **) &old_openmemory) != ELE7EN_OK) {
        __android_log_print(ANDROID_LOG_ERROR, TAG, "register hook failed");
        return;
    }
...
//有些手机的openmemory和我这里的不一样，你要用ida看一下具体的名字，然后修改一下
void hook_23_plus() {
...
JNIEXPORT void JNICALL Java_com_alex_hookopenmomory_Dumper_dump(JNIEnv *env, jclass clazz) {
